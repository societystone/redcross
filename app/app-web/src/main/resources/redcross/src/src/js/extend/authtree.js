	...
	obj = {
		...
		renderAuth: function(tree, dept, opt){
			...
			layui.each(tree, function(index, item){
				var hasChild = item.childs ? 1 : 0;
				// 注意：递归调用时，this的环境会改变！
				var append = hasChild ? obj.renderAuth(item.childs, dept+1, opt) : '';
		...
		// 设置选中数据
		setChecked: function(dst,value){
			$(dst).find('input[type="checkbox"][value="'+value+'"]:not(:checked)').prop('checked', true);
			form.render('checkbox');
			obj.autoWidth(dst);
			// 变动则存一下临时状态
			obj._saveNodeStatus(dst);
		...
		// 设置不选中数据
		setNotChecked: function(dst,value){
			$(dst).find('input[type="checkbox"][value="'+value+'"]:checked').prop('checked', false);
			form.render('checkbox');
			obj.autoWidth(dst);
			// 变动则存一下临时状态
			obj._saveNodeStatus(dst);
