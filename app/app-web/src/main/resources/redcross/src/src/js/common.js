...
layui.define([ 'laytpl', 'layer', 'element', 'util', 'form', 'table', 'config',
		'laydate' ], function(exports) {
	...
	var func = {
		...
		loadLoaclData : function(refType) {
			...
			if (refType == this.Constants.User) {
				...
				var tableName = refType
						.substr(this.Constants.RemoteTableColumn.length);
				$.ajax({
					method : "GET",
					contentType : 'application/json',
		...
		/**
		 * 获取码值 refType 码值类型 status 状态，不传或者null代表所有状态；1启用；0禁用
		 */
				...
				for (var i = 0; i < data.length; i++) {
					if (data[i].value == checkedValue) {
						html += "<input type='radio' name='" + name
								+ "' value='" + data[i].value + "' title='"
								+ data[i].title + "' checked>";
					} else {
						html += "<input type='radio' name='" + name
								+ "' value='" + data[i].value + "' title='"
								+ data[i].title + "'>";
